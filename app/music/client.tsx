...
export function MusicPageClient({
  hymns,
  massSectionLabels: _msLabels,
  massSectionOrder: _msOrder,
  traditionLabels,
}: {
  hymns: Hymn[];
    ...
    <>
      ...
      <Section size="lg">
        <SectionHeader
          ...
          subtitle="A selection of our favourite sacred hymns"
        />
        {hymns.length === 0 ? (
          <p className="text-center text-text-muted py-8">
            No hymns in the public repertoire yet.
          </p>
        ) : (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {hymns.map((song, index) => (
              <motion.div
                key={song.id}
                  ...
                  <CardContent>
                    <div className="flex items-start justify-between gap-3">
                      ...
                      {song.tradition && (
                        ...
                        >
                          {traditionLabels[song.tradition] ?? song.tradition}
                        </span>
                      )}
                    </div>
                    {song.language && (
