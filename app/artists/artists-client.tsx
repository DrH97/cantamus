...
export function ArtistsClient({
  ...
  corMembers: Artist[];
  conductors: Artist[];
  voicePartMembers: Record<string, Artist[]>;
}) {
  return (
    <>
      {/* Hero Section */}
              ...
              <Card className="h-full text-center overflow-hidden">
                <div className="relative h-32 bg-gradient-to-br from-primary/20 via-secondary/10 to-royal-purple/20">
                  <div className="absolute inset-0 flex items-center justify-center">
                    {member.photoUrl ? (
                      <div className="w-20 h-20 overflow-hidden border-2 border-primary/30 group-hover:border-primary transition-colors rounded-full">
                        <Image
                          src={member.photoUrl}
                          alt={member.name}
                          width={80}
            ...
            >
              <Card className="w-64 text-center overflow-hidden">
                <div className="relative h-40 bg-gradient-to-br from-royal-purple/30 via-primary/20 to-secondary/20">
                  <div className="absolute inset-0 flex items-center justify-center">
                    {member.photoUrl ? (
                      <div className="w-24 h-24 overflow-hidden border-2 border-primary/30 group-hover:border-primary group-hover:glow-gold-sm transition-all rounded-full">
                        <Image
                          src={member.photoUrl}
                          alt={member.name}
                          width={96}
      ...
      {voiceSections.map((voicePart, sectionIndex) => {
        const members = voicePartMembers[voicePart] ?? [];
                ...
                >
                  <Card className="h-full">
                    <CardContent className="flex items-center gap-4">
                      {member.photoUrl ? (
                        <div className="h-12 w-12 flex-shrink-0 overflow-hidden rounded-full border border-primary/20">
                          <Image
                            src={member.photoUrl}
                            alt={member.name}
                            width={48}
